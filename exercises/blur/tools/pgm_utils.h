 ...
 *
 */
void read_pgm_image(void **image, int *maxval, int *xsize, int *ysize,
                    const char *image_name) {
  FILE *image_file;
  ...
  *xsize = *ysize = *maxval = 0;
  ...
  if (k > 0) {
    ...
    if (k < 3)
      n = fscanf(image_file, "%d%*c", maxval);
  } else {
 ...
 */
{
  ...
  unsigned short int *sImage; // the image when a two bytes are used for each pixel
  void *ptr;
  ...
  {
    sImage = (unsigned short int *)calloc(xsize * ysize, sizeof(unsigned short int));
    unsigned short int _maxval = swap((unsigned short int)maxval);
    int idx = 0;
    for (int yy = 0; yy < ysize; yy++) {
      unsigned short int value = (unsigned short int)(minval + yy * delta);
      // swap high and low bytes, the format expect big-endianism
